function [figHandle] = CapPlot(dampSP, cap, level1Bound, level2Bound, level3Bound, level, plotTitle, plotText)
% Plot the control anticipation parameter requirements with boundaries.
%
%Usage:  [figHandle] = CapPlot(dampSP, cap, level1Bound, level2Bound, level3Bound, level, plotTitle, plotText)
%
%Inputs:
...
% plotTitle   - plot title ['CAP Requirements']
% plotText    - plot text []
%
%Outputs:
...
if isempty(plotTitle), plotTitle = 'CAP Requirements'; end
...
% Plot data points
loglog(...
    dampSP(level==3), cap(level==3), '*b',...
    dampSP(level==2), cap(level==2), '*m',...
    dampSP(level==1), cap(level==1), '*r',...
    dampSP(level==0), cap(level==0), '*k');
...
ylabel('Control Anticipation Parameter', 'Interpreter', 'none');
