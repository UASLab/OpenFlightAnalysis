function [xxP, indxSegRef, freq] = SpectogramEst(x, lengthSeg, shiftSeg, freqVec, freqRate, winType, smoothFactor)
% Compute the Spectrogram of time history data.
%
%Usage:  [xxP, xComplex, freq] = SpectogramEst(x, lengthSeg, shiftSeg, freqVec, freqRate, winType, smoothFactor);
%
% Inputs:
%  x            - time history data
...
for indxSeg = 1:numSeg
    ...
    [xxP(indxSeg, :), ~, freq] = SpectEstCZT(xSeg(indxSeg, :), freqVec, freqRate, winType, smoothFactor);
end
