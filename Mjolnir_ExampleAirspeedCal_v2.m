...
%     param{iSeg}.s_PB_rad.data = [0.0; -6.0; -3.0] * d2r;
    
    param{iSeg}.v_PA_P_mps.errorType = 'ScaleBias+';
    param{iSeg}.v_PA_P_mps.bias = 0.0;
    param{iSeg}.v_PA_P_mps.K = 1.00;
...
optParam.seg{indxOptSeg}.ub = [10; 10; 0]; optParam.seg{indxOptSeg}.lb = -optParam.seg{indxOptSeg}.ub;
...
%% Compute the 5-Hole probe measurements at the CG
alphaErr = NaN(numSeg, 1);
betaErr = NaN(numSeg, 1);
for iSeg = 1:numSeg
    
    param{iSeg}.s_5B_rad.data = 0*[4.1204; 0.9194; -0.6328] * d2r;
    param{iSeg}.p5Tip_Pa.K = 1.0;%1.2789;
    param{iSeg}.p5Tip_Pa.bias = 0*-0.0019;
    param{iSeg}.p5Alpha_Pa.K = 0.0642;
    param{iSeg}.p5Alpha_Pa.bias = 0;
    param{iSeg}.p5Beta_Pa.K = 0.0642;
    param{iSeg}.p5Beta_Pa.bias = 0;
    
    [seg{iSeg}.vEst5_BA_B_mps, seg{iSeg}.vEst5_BA_L_mps] = Press5HoleCal(seg{iSeg}.p5Tip_Pa, seg{iSeg}.p5Alpha_Pa, seg{iSeg}.p5Beta_Pa, seg{iSeg}.sNav_BL_rad, param{iSeg});
    seg{iSeg}.vMag5_BA_mps = sqrt(seg{iSeg}.vEst5_BA_B_mps(1,:).^2 + seg{iSeg}.vEst5_BA_B_mps(2,:).^2 + seg{iSeg}.vEst5_BA_B_mps(3,:).^2);
    
    seg{iSeg}.alpha5_BA_deg = atan2d(seg{iSeg}.vEst5_BA_B_mps(3,:), seg{iSeg}.vEst5_BA_B_mps(1,:));
    seg{iSeg}.beta5_BA_deg = asind(seg{iSeg}.vEst5_BA_B_mps(2,:) ./ seg{iSeg}.vMag5_BA_mps);
    
    alphaErr(iSeg) = norm(seg{iSeg}.alpha_BA_deg - seg{iSeg}.alpha5_BA_deg, 2);
    betaErr(iSeg) = norm(seg{iSeg}.beta_BA_deg - seg{iSeg}.beta5_BA_deg, 2);
end
norm(alphaErr, 2)
norm(betaErr, 2)
...
optPlotCell.legNames = {{'Measured', 'Estimated', '5-Hole', 'Inertial Derived, Mean Wind Compensated'},{'Measured', 'From Mean Wind Compensated', '5-Hole'},{'Measured', 'From Mean Wind Compensated', '5-Hole'}};
optPlotCell.titleStr = 'Inflow Conditions';

for iSeg = 1:numSeg
    PlotCell({seg{iSeg}.time_s/60}, ...
        {{seg{iSeg}.vMeasMag_PA_mps, seg{iSeg}.vCalMag_BA_mps, seg{iSeg}.vMag5_BA_mps, seg{iSeg}.vMag_BA_mps}, ...
        {seg{iSeg}.alphaCal_BA_deg, seg{iSeg}.alpha_BA_deg, seg{iSeg}.alpha5_BA_deg}, ...
        {seg{iSeg}.betaCal_BA_deg, seg{iSeg}.beta_BA_deg, seg{iSeg}.beta5_BA_deg}}, ...
        optPlotCell);
    hold on;
end
