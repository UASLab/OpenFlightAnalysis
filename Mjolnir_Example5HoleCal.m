...
flt{1}.param.fltNum = 6;
flt{1}.param.fltFile = fullfile(basePath, 'MjolnirFLT06', 'MjolnirFLT06.mat');

flt{2}.param.vehName = 'Mjolnir';
flt{2}.param.apType = 'Goldy3';
flt{2}.param.config = '1';
flt{2}.param.fltNum = 7;
flt{2}.param.fltFile = fullfile(basePath, 'MjolnirFLT07', 'MjolnirFLT07.mat');
...
param{iSeg} = flt{indxFlt}.param;
param{iSeg}.indxFlt = indxFlt;
param{iSeg}.indxSeg = flt{indxFlt}.seg.turn{5}.indx;

iSeg = iSeg+1; indxFlt = 2;
param{iSeg} = flt{indxFlt}.param;
param{iSeg}.indxFlt = indxFlt;
param{iSeg}.indxSeg = flt{indxFlt}.seg.aircal.indx;
...
for iSeg = 1:numSeg
    ...
    param{iSeg}.s_5B_rad.data = [5.0; 0.0; 0.0] * d2r;

    param{iSeg}.p5Tip_Pa.errorType = 'ScaleBias+';
    param{iSeg}.p5Tip_Pa.bias = 0;
    param{iSeg}.p5Tip_Pa.K = 1.2;
    
    param{iSeg}.p5Alpha_Pa.errorType = 'ScaleBias+';
    param{iSeg}.p5Alpha_Pa.bias = 0;
    param{iSeg}.p5Alpha_Pa.K = 0.0642;
    
    param{iSeg}.p5Beta_Pa.errorType = 'ScaleBias+';
    param{iSeg}.p5Beta_Pa.bias = 0;
    param{iSeg}.p5Beta_Pa.K = 0.0642;
    
    % Transform Airspeed Measures
    [seg{iSeg}.vTemp_BA_B_mps, seg{iSeg}.vTemp_BA_L_mps] = ...
        Press5HoleCal(seg{iSeg}.p5Tip_Pa, seg{iSeg}.p5Alpha_Pa, seg{iSeg}.p5Beta_Pa, seg{iSeg}.sNav_BL_rad, param{iSeg});
...
clear optParam;
indxOptStatic = 1;
optParam.static{indxOptStatic}.field = 's_5B_rad.data';
optParam.static{indxOptStatic}.len = 3;
optParam.static{indxOptStatic}.ub = [30; 10; 10]*d2r; optParam.static{indxOptStatic}.lb = -optParam.static{indxOptStatic}.ub;

indxOptStatic = indxOptStatic + 1;
optParam.static{indxOptStatic}.field = 'p5Tip_Pa.K';
optParam.static{indxOptStatic}.len = 1;

indxOptStatic = indxOptStatic + 1;
optParam.static{indxOptStatic}.field = 'p5Tip_Pa.bias';
optParam.static{indxOptStatic}.len = 1;

indxOptStatic = indxOptStatic + 1;
optParam.static{indxOptStatic}.field = 'p5Alpha_Pa.K';
optParam.static{indxOptStatic}.len = 1;
optParam.static{indxOptStatic}.ub = 1.5 * param{iSeg}.p5Alpha_Pa.K; optParam.static{indxOptStatic}.lb = 0.5 * param{iSeg}.p5Alpha_Pa.K;

indxOptStatic = indxOptStatic + 1;
optParam.static{indxOptStatic}.field = 'p5Alpha_Pa.bias';
optParam.static{indxOptStatic}.len = 1;
optParam.static{indxOptStatic}.ub = 10 * d2r; optParam.static{indxOptStatic}.lb = -optParam.static{indxOptStatic}.ub;

indxOptStatic = indxOptStatic + 1;
optParam.static{indxOptStatic}.field = 'p5Beta_Pa.K';
optParam.static{indxOptStatic}.len = 1;
optParam.static{indxOptStatic}.ub = 1.5 * param{iSeg}.p5Beta_Pa.K; optParam.static{indxOptStatic}.lb = 0.5 * param{iSeg}.p5Beta_Pa.K;

indxOptStatic = indxOptStatic + 1;
optParam.static{indxOptStatic}.field = 'p5Beta_Pa.bias';
optParam.static{indxOptStatic}.len = 1;
optParam.static{indxOptStatic}.ub = 10 * d2r; optParam.static{indxOptStatic}.lb = -optParam.static{indxOptStatic}.ub;
...
optParam.seg{indxOptSeg}.ub = [10; 10; 10]; optParam.seg{indxOptSeg}.lb = -optParam.seg{indxOptSeg}.ub;
...
% Setup the Optimizer
optProb.objective = @(optParamVal) double(Press5HoleCalCost(seg, param, optParam, optParamVal));
...
% Call the Optimizer
% [cost] = Press5HoleCalCost(seg, param, optParam, optParam.val);
...
[~, seg, param] = Press5HoleCalCost(seg, param, optParam, optParamVal);
...
optPlotCell.legNames = repmat({{'GPS', 'Nav Filter', 'Optimized Air'}}, 3, 1);
...
optPlotCell.legNames = {{'Measured', 'Estimated', 'Inertial Derived, Mean Wind Compensated'},{'Measured', 'From Mean Wind Compensated'},{'Measured', 'From Mean Wind Compensated'}};
...
for iSeg = 1:numSeg
    ...
    subplot(2,1,1)
    plot(seg{iSeg}.p5Tip_Pa, seg{iSeg}.vMag_BA_mps, '*'); grid on;
    ylabel('Airpeed Estimate (m/s)');
    hold on;
    
    subplot(2,1,2)
    plot(seg{iSeg}.p5Tip_Pa, seg{iSeg}.vCalErrorMag_BA_mps, '*'); grid on;
    ylabel('Airpeed Error RSS Estimate (m/s)');
    xlabel('Measured Tip Pressure (Pa)')
    hold on;
end
linkaxes(findobj(gcf, 'Type', 'axes'), 'x');

pV = 0:1:500; pA = 0*pV; pB = 0*pV;
v = real(Press5HoleCal(pV, pA, pB, seg{1}.sNav_BL_rad, param{1}));
subplot(2,1,1);
plot(pV, sqrt(v(1,:).^2 + v(2,:).^2 + v(3,:).^2), 'k');

figure;
for iSeg = 1:numSeg
    % Plot Alpha Error Estimate
    subplot(2,1,1)
    plot(seg{iSeg}.p5Alpha_Pa, seg{iSeg}.alpha_BA_deg, '*'); grid on;
    ylabel('Angle of Attack Estimate (deg)');
    hold on;
    
    subplot(2,1,2)
    plot(seg{iSeg}.p5Alpha_Pa, seg{iSeg}.alpha_BA_deg - seg{iSeg}.alphaCal_BA_deg, '*'); grid on;
    ylabel('Alpha Error RSS Estimate (deg)');
    xlabel('Measured Alpha Pressure (Pa)')
    hold on;
end
linkaxes(findobj(gcf, 'Type', 'axes'), 'x');

pA = -500:1:500; pV = repmat(250, size(pA)); pB = 0*pA;
v = real(Press5HoleCal(pV, pA, pB, seg{1}.sNav_BL_rad, param{1}));
a = atan2d(v(3,:), v(1,:));
subplot(2,1,1);
plot(pA, a, 'k');
...
for iSeg = 1:numSeg
    % Plot Alpha Error Estimate
    subplot(2,1,1)
    plot(seg{iSeg}.p5Beta_Pa, seg{iSeg}.beta_BA_deg, '*'); grid on;
    ylabel('Angle of Sideslip Estimate (deg)');
    hold on;
    
    subplot(2,1,2)
    plot(seg{iSeg}.p5Beta_Pa, seg{iSeg}.beta_BA_deg - seg{iSeg}.betaCal_BA_deg, '*'); grid on;
    ylabel('Beta Error RSS Estimate (deg)');
    xlabel('Measured Beta Pressure (Pa)')
    hold on;
end
linkaxes(findobj(gcf, 'Type', 'axes'), 'x');

pB = -500:1:500; pV = repmat(250, size(pB)); pA = 0*pB;
v = real(Press5HoleCal(pV, pA, pB, seg{1}.sNav_BL_rad, param{1}));
b = asind(v(2,:) ./ sqrt(v(1,:).^2 + v(2,:).^2 + v(3,:).^2));
subplot(2,1,1);
plot(pB, b, 'k');

return
%% Save
save('Mjolnir_5HoleCal.mat', 'param', 'seg', 'optParam');
