...
for iSeg = 1:numSeg
    ...
    param{iSeg}.pPitotDiff_Pa.errorType = 'ScaleBias+';
    param{iSeg}.pPitotDiff_Pa.bias = -0.0098;
    param{iSeg}.pPitotDiff_Pa.K = 1.0515;
    
    % Transform Airspeed Measures
    [seg{iSeg}.vTemp_BA_B_mps, seg{iSeg}.vTemp_BA_L_mps] = ...
        PitotCal(seg{iSeg}.pPitotDiff_Pa, seg{iSeg}.sNav_BL_rad, param{iSeg});
...
optParam.static{indxOptStatic}.field = 'pPitotDiff_Pa.K';
optParam.static{indxOptStatic}.len = 1;
optParam.static{indxOptStatic}.ub = 100; optParam.static{indxOptStatic}.lb = -optParam.static{indxOptStatic}.ub;

indxOptStatic = indxOptStatic + 1;
optParam.static{indxOptStatic}.field = 'pPitotDiff_Pa.bias';
optParam.static{indxOptStatic}.len = 1;
optParam.static{indxOptStatic}.ub = 100; optParam.static{indxOptStatic}.lb = -optParam.static{indxOptStatic}.ub;
...
% Setup the Optimizer
optProb.objective = @(optParamVal) double(PitotCalCost(seg, param, optParam, optParamVal));
...
% Call the Optimizer
% [cost] = PitotCalCost(seg, param, optParam, optParam.val);
...
[~, seg, param] = PitotCalCost(seg, param, optParam, optParamVal);
...
for iSeg = 1:numSeg
    ...
    subplot(2,1,1)
    plot(seg{iSeg}.pPitotDiff_Pa, seg{iSeg}.vMag_BA_mps, '*'); grid on;
    ylabel('Airpeed Estimate (m/s)');
    hold on;
    
    subplot(2,1,2)
    plot(seg{iSeg}.pPitotDiff_Pa, seg{iSeg}.vCalErrorMag_BA_mps, '*'); grid on;
    ylabel('Airpeed Error RSS Estimate (m/s)');
    xlabel('Measured Diff Pressure (Pa)')
    hold on;
end
...
p = 0:1:1000; 
subplot(2,1,1);
plot(p, max(real(PitotCal(p, seg{1}.sNav_BL_rad, param{1}))), 'k');
...
return
%% Save
save('Mjolnir_PitotCal.mat', 'param', 'seg', 'optParam');
