...
for iSeg, seg in enumerate(oDataSegs):
    ...
    freq, Tey, Cey, Pee, Pyy, Pey, TeyUnc, Pee_N, Pyy_N = FreqTrans.FreqRespFuncEstNoise(eList[iSeg], outList[iSeg], optSpec)
...
if True:
    ...
    for iIn, inName in enumerate(inPlot):
        for iOut, outName in enumerate(outPlot):
            ...
            fig = None
            for iSeg in range(0, len(oDataSegs)):
                fig = FreqTrans.PlotBode(freq_hz[iIn], gain_mag[iSeg][iOut, iIn], phase_deg[iSeg][iOut, iIn], C[iSeg][iOut, iIn], gainUnc_mag = gainUnc_mag[iSeg][iOut, iIn], dB = False, fig = fig, fmt = '*--', label = oDataSegs[iSeg]['Desc'])
