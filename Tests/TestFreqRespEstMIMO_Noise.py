...
_, out[1][0], _ = signal.lsim2(sys[1][0], exc[0]+xD[0], time_s)
_, out[0][1], _ = signal.lsim2(sys[0][1], exc[1]+xD[1], time_s)
...
y = [0] * 2
iOut = 0
y[iOut] = out[iOut][0] + out[iOut][1] + yN[iOut]
iOut = 1
y[iOut] = out[iOut][0] + out[iOut][1] + yN[iOut]
...
plt.figure(0)
iIn = 0; iOut = 0
plt.semilogx(freq_hz[0], 20*np.log10(Pxx[iIn]), '*b', label='P Excitation')
plt.semilogx(freqE_hz[0], 20*np.log10(Pee[iIn]), '*m', label='P Excitation @ Null')
plt.semilogx(freq_hz[0], 20*np.log10(Pyy[iOut,iIn]), '*r-', label='P Output')
plt.semilogx(freqN_hz[0], 20*np.log10(Pnn[iOut]), '.g--', label='P Disturbance')
#plt.semilogx(freq_hz[0], 20*np.log10(Pyy_N[iOut,iIn]), '*k', label='P Disturbance @ Null')
plt.grid(True)
plt.legend()
